...
import { fetchCompetitionWindow, splitUpcomingAndFinished, type FDMatch } from "./footballData";
  ...
  for (const league of ACTIVE_LEAGUES) {
    ...
    const getSnapshot = (teamId: number, teamName: string): TeamSnapshot => {
      if (!teamSnapshotCache.has(teamId)) {
        teamSnapshotCache.set(teamId, buildTeamSnapshotFromLeagueResults(teamId, teamName, finished));
      }
    ...
    for (const fixture of upcoming as FDMatch[]) {
      ...
      matches.push({
        ...
        context: { competitionName: league.name, importance: "normal", leagueAvgHomeGoals: 1.5, leagueAvgAwayGoals: 1.15 },
        odds: {
          home: odds.home, draw: odds.draw, away: odds.away,
          over15: odds.over15, under15: odds.under15, over25: odds.over25, under25: odds.under25,
          bttsYes: undefined, bttsNo: undefined,
        },
      });
    }
  }
